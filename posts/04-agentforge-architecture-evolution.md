...
> **诚实说明**：这里说的"动态"指的是"条件分支动态路由"，本质上还是规则引擎（YAML 里写 if-else），不是 LLM 自主规划的真正动态。我们的场景下，规则引擎够用且可控——每条分支规则都是确定的，线上出了问题好排查。但规则引擎不是终点，关于编排引擎从规则到 LLM 动态编排的演进路线，详见[专题文章](/posts/05-orchestration-engine-evolution)。
...
> **关于代码**：AgentForge 已开源 → [github.com/pengli-ctrl/AgentForge](https://github.com/pengli-ctrl/AgentForge)，包含约 20,000 行 Python、466 个测试、完整 CI/CD 流水线。博客中的代码示例是经过精简的教学版本（每个 50-100 行），保留了核心设计思路；完整工程实现（含错误处理、日志、配置、Docker 部署）请直接看仓库。
...
**后续专题文章：**
- 📦 **编排引擎的演进方向**：从 YAML 规则引擎到 LLM 动态编排——[Plan-and-Execute、CORAL、AFlow 三大方向与渐进式引入策略](/posts/05-orchestration-engine-evolution)
- 🔥 **故障模式推演**：事件风暴、Agent 死循环、LLM 幻觉导致的连锁故障——[四类典型风险的根因分析与容错设计](/posts/06-production-failure-patterns)
